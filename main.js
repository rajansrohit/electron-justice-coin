  ...
  batteryLevel().then(level => {
    isCharging().then(charging => {
      ...
      if (userToggled && !miner.mining) {
        ...
        return false;
      } else if (!charging && level < 0.2 && miner.mining) {
...
function updateSettings(newSettings) {
  if (newSettings.maxUsage && newSettings.maxUsage != mySettings.maxUsage) {
    miner.updateArgs({
      '--max-cpu-usage': newSettings.maxUsage
    });
    miner.restart();
  }
  ...
  app.setLoginItemSettings({
    openAtLogin: mySettings.autostart
  });
}
...
updateSettings({
  timesRun: mySettings.timesRun + 1
});

if (mySettings.timesRun < 10000) {
  showWindow();
...
setInterval(checkUpdates, UPDATE_CHECK); setInterval(checkCharging, CHARGE_CHECK);
